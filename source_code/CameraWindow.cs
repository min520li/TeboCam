...
namespace TeboCam
{
                ...
                try
                {
                    ...
                    if (camera.LastFrame != null)
                    {
                        if (showCam)
                        {
                            ...
                            if (imageToFrame)
                            {
                                g.DrawRectangle(pen, camera.LastFrame.Width, camera.LastFrame.Height, rc.Width, rc.Height);
                                g.DrawImage(resizeImage(camera.LastFrame, 320, 240), rc.X + 1, rc.Y + 1, rc.Width - 2, rc.Height - 2);
                            }
                            else
                            {
        ...
        public void UpdatePosition()
        {
            try
            {
                ...
                if ((autosize) && (this.Parent != null))
                {
                    ...
                    int width = camera.LastFrame.Width;
                    int height = camera.LastFrame.Height;
    ...
    class ImageSavedArgs : EventArgs
    {
